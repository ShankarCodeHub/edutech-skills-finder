...
import { FaTimes, FaQuestionCircle, FaCheckCircle } from "react-icons/fa";
...
export default function Quiz({ isOpen, onClose, onComplete }) {
  ...
  const [wide, setWide] = useState(false); // user-controlled wider layout
  const qEls = useRef({});

  // Build questions from selected interests
  const interestQuestions = useMemo(() => {
    const qs = [];
    for (const interest of selectedInterests) {
      if (INTEREST_QUESTIONS[interest]) {
        qs.push(...INTEREST_QUESTIONS[interest]);
      }
    }
    return qs;
  }, [selectedInterests]);
  ...
  const questions = useMemo(
    () => [...interestQuestions, ...COMMON_QUESTIONS],
    [interestQuestions]
  );

  const allAnswered = selectedInterests.length > 0 && questions.every((q) => !!answers[q.key]);
  ...
  const canSubmit = selectedInterests.length > 0 && !submitting;

  // Dynamic width based on number of selected interests and optional wide toggle
  const widthClass = useMemo(() => {
    if (wide) return 'max-w-6xl min-w-[600px]';
    const n = selectedInterests.length;
    if (n <= 2) return 'max-w-2xl min-w-[420px]';
    if (n <= 4) return 'max-w-3xl min-w-[520px]';
    if (n <= 6) return 'max-w-4xl min-w-[650px]';
    return 'max-w-6xl min-w-[800px]';
  }, [selectedInterests.length, wide]);
  ...
  // Prefill interests from profile if available
  useEffect(() => {
    if (!isOpen) return;
    const token = localStorage.getItem('auth_token');
    if (!token) return;
    if (selectedInterests.length > 0) return; // already selected
    (async () => {
      try {
        ...
        if (Array.isArray(data.interests) && data.interests.length > 0) {
          setSelectedInterests(data.interests.filter(opt => INTEREST_OPTIONS.includes(opt)));
        }
      } catch {}
    })();
  ...
  useEffect(() => {
    try {
      localStorage.setItem("quiz_state", JSON.stringify({ selectedInterests, answers }));
    } catch {}
  ...
  const submit = useCallback(async (allowPartial = false) => {
    if (!allAnswered && !allowPartial) {
      setError("Please select at least one interest and answer all questions.");
      return;
    }
    setError("");
    setSubmitting(true);
    ...
    try {
      ...
      try {
        ...
        res = await fetch("/api/quiz", {
          ...
          body: JSON.stringify({ answers, selectedInterests }),
        });
      } catch (err) {
  ...
  }, [allAnswered, answers, selectedInterests, onComplete, onClose]);
  ...
  const toggleInterest = (interest) => {
    setSelectedInterests(prev => {
      ...
      return exists ? prev.filter(x => x !== interest) : [...prev, interest];
    });
  };

  return !isOpen ? null : (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-sm p-3">
  <div className={`relative w-full ${widthClass} overflow-hidden rounded-xl border border-purple-200/60 bg-white/70 backdrop-blur-xl shadow-xl`}>
        {/* Top progress bar */}
  ...
  <div className="p-4 flex flex-col min-h-[70vh] relative">
          {/* Header */}
          <div className="flex items-start justify-between gap-3 mb-3">
            <div>
              ...
              <p className="text-sm text-purple-700/80 mt-1">
                {selectedInterests.length > 0 ? `${questions.length} questions. ${answeredCount}/${questions.length} answered` : 'Select your interests to begin'}
              </p>
            </div>
            <div className="flex items-center gap-2">
              <button
                type="button"
                onClick={() => setWide((w) => !w)}
                className="px-2.5 py-1.5 rounded-lg border border-purple-300 text-purple-800 hover:bg-purple-50 text-xs font-semibold"
                title="Toggle wider layout"
              >
                {wide ? 'Normal width' : 'Wider'}
              </button>
              <button
                onClick={onClose}
                className="inline-flex items-center justify-center h-9 w-9 rounded-xl border border-purple-300/60 bg-white/70 text-purple-800 hover:text-red-600 shadow-sm"
          ...
          {/* Interest selector */}
          <div className="mb-4">
            <label className="font-semibold text-purple-900 mb-2 block select-none">Select your interests/skills (one or more)</label>
              ...
              {INTEREST_OPTIONS.map((interest) => {
                ...
                return (
                  <button
                    ...
                    className={`px-2.5 py-1 rounded-full border text-xs sm:text-sm font-semibold transition ${
                      selected
                        ? 'bg-purple-600 text-white border-purple-600'
                        : 'bg-white/80 text-purple-900 border-purple-300 hover:bg-purple-50'
                    }`}
                  >
                    {interest}
            ...
            <p className="mt-2 text-xs text-purple-700/80">
              Each interest generates 10 questions. Total: {interestQuestions.length} interest questions + 2 common = {questions.length} questions
            </p>
          </div>

          {/* Step dots */}
          {questions.length > 0 && (
            <div className="flex items-center gap-2 mb-3 overflow-x-auto pb-1.5">
              {questions.map((q, idx) => {
              ...
              <span className="ml-2 text-xs text-purple-700/80 shrink-0">
          ...
          {/* Questions */}
          <div className="flex-1 min-h-0 max-h-[60vh] overflow-auto pr-2 scroll-smooth pb-32">
            <div className={`grid grid-cols-1 md:grid-cols-2 ${selectedInterests.length >= 4 ? 'xl:grid-cols-3' : 'xl:grid-cols-2'} gap-4`}>
              {questions.map((q, idx) => (
                <div
                  key={q.key}
                  ref={(el) => (qEls.current[q.key] = el)}
                  className="rounded-xl border border-purple-200 bg-white/70 p-3 hover:shadow-lg transition-shadow duration-300 cursor-pointer select-none"
                >
                  <div className="flex items-center gap-2 font-semibold text-purple-900 mb-2 text-base">
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
                    {q.options.map((opt) => {
                      ...
                      return (
                        <label
                          key={opt}
                          className={`inline-flex items-center gap-2 rounded-lg border px-3 py-1.5 cursor-pointer transition-all ${
                            selected
          ...
          {/* Actions */}
          <div className="fixed left-1/2 -translate-x-1/2 z-30 w-[calc(100%-2rem)]"
               style={{bottom: 'max(env(safe-area-inset-bottom,0px),1.5rem)'}}>
            <div className="flex items-center justify-between gap-3 rounded-xl border border-purple-200 bg-white/95 shadow-lg px-4 py-3">
            <div className="flex items-center gap-2">
              <button
                type="button"
                onClick={() => gotoIndex(activeIndex - 1)}
                disabled={activeIndex <= 0}
                className="px-3.5 py-1.5 rounded-lg border border-purple-300 text-purple-800 bg-white hover:bg-purple-50 transition disabled:opacity-50"
              >
                Prev
              </button>
              <button
            ...
            <button
              onClick={onClose}
              className="px-4 py-1.5 rounded-lg border border-purple-300 text-purple-800 bg-white hover:bg-purple-50 transition select-none"
            >
              Cancel
            </button>
            <div className="flex items-center gap-2">
              <span className="hidden sm:inline text-xs text-purple-700/80 select-none">
                {answeredCount}/{questions.length} answered
              </span>
              <button
                disabled={!canSubmit}
                onClick={() => submit(!allAnswered)}
                className="group relative inline-flex items-center justify-center gap-2 px-4 py-2 rounded-lg bg-gradient-to-r from-purple-600 to-purple-500 text-white font-semibold shadow-md shadow-purple-600/20 hover:shadow-lg hover:shadow-purple-700/30 transition disabled:opacity-75 disabled:cursor-not-allowed select-none"
                title={allAnswered ? 'Submit all answers' : 'Submit now with partial answers'}
              >
                {submitting ? "Submitting..." : allAnswered ? "Submit All" : "Submit Now"}
